...
local function fireMainMenuLeavingHooks()
	leaving = true
	modApi.events.mainMenuLeaving:fire()
end
...
local function createUi(root)
	...
	btnContinue.mousedown = function(self, x, y, button)
		if button == 1 and not isWindowVisible() and not leaving then
			modApi.events.continueClick:fire()
		end
	...
	btnNewGame.mousedown = function(self, x, y, button)
		if button == 1 and not isWindowVisible() and not leaving then
			modApi.events.newGameClick:fire()
		end
		return Ui.mousedown(self, x, y, button)
	end
...
modApi.events.newGameClick:subscribe(function()
...
modApi.events.continueClick:subscribe(function()
	fireMainMenuLeavingHooks()
end)

modApi.events.mainMenuEntered:subscribe(function(screen, wasHangar, wasGame)
	leaving = false
